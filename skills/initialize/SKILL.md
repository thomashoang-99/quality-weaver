...
**Allowed state:** No `.quality-weaver/` workspace exists under `<project-path>`.
...
**Output artifact:** `.quality-weaver/config.yaml`, with the remaining workspace structure managed by the CLI.
...
- Do not write `.quality-weaver/state.json`; only the CLI owns state.
